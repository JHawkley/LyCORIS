...
class FullModule(LycorisBaseModule):
    ...
    def make_weight(self, scale=1, device=None, diff=False):
        ...
        org_weight = self.org_weight.to(device=diff_w.device, dtype=diff_w.dtype)
        weight = org_weight + diff_w
        if self.org_bias is not None and diff_b is not None:
            org_bias = self.org_bias[0].to(device=diff_b.device, dtype=diff_b.dtype)
            bias = org_bias + diff_b
        else:
